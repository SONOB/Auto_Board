...
#include <RF24.h>
#include <Servo.h>
#define SPMAX 80 //모터 최대출력값 지정 BLDC모터이기 때문에 75도 상당히 위험함 따라서 낮춰쓰는것을 추천
#define SPMIN 60
Servo motor1,motor2;
...
void loop() {
  while (radio.available()) {
    ...
    if (val1 > SPMAX || val2 > SPMAX){ //속도가 지정한 최대값이 넘으면 넘어가지 않도록 조정
      val1 = 80;
      val2 = 80;
    }
    else if (val1 < SPMIN || val2 < SPMIN){ //속도가 지정한 최소값보다 낮으면 더 내려가지 않도록 조정
      val1 = 60;
      val2 = 60;
    }
    else{
